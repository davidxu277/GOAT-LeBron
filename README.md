...
赛题要的是「最终模型输出 / checkpoint」—— 模型对数据吐出的两列概率，
或者模型文件本身。**不是分数，不是配置，不是日志。**

一场跑完，`logs/` 下的东西**没有一样是它**。它得这样产出来：

```
logs/best_pipeline 的配方  →  照着重跑一次  →  预测结果 / checkpoint  ← 交付物 #4
```

> ⏳ **「导出预测结果」这段代码还没写** —— 提交格式（输出 Schema）要等主办方的
> Starter Kit。列名叫什么、带不带 `sample_id`、CSV 还是 parquet，现在写就是猜。
> 拿到 Schema 后补上，`finalize` 就能连它一起产出。
>
> 在那之前**配方存住了是最关键的一步**：配方丢了才真回不去，预测结果随时能重跑。

### 一场跑完 `logs/` 下有什么

| 文件 | 是什么 |
|---|---|
| `rounds.jsonl` | **交付物 #3** 逐轮日志：假设 / 代码全文 / 指标 / 错误恢复 / 干预 |
| `narrative.md` | **交付物 #3** 的人话版：一整场压成一条故事线 |
| `session_summary.json` | **交付物 #5** 结果表：最佳分、与基线的差值、token 总量、GPU 小时 |
| `snapshots/<run_id>/round_XX.json` | 每一轮的配置 + 零件清单 —— 还原任意一轮靠它 |
| `best_report.json` | 最佳那轮的**分数记录**（两个 AUC、分桶指标、种子）。**不是提交物**，是佐证结果表里那两个数从哪来的 |
...
可以直接提交）：逐轮日志、叙事、结果表、分数记录、评委可读的看板，
以及**最佳那一轮的配置与零件**（`best_pipeline/`）。

**注意：这一步产不出交付物 #4。** 它给的是配方，模型还得照着配方重跑一次。
...
磁盘上只剩最后那个叠加态，第 5 轮的样子早被盖掉了 —— 没有它，
"验证集最佳的那一版"就永远交不出来了。
