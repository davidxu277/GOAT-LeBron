...
> The doctor says the kid is bad at maths. You hire a maths tutor. The term score goes
> up 3 points. But maths is unchanged — the gain came from an easy literature paper.
> Record that as "tutoring works" and you will keep buying maths tutoring, and keep
> getting further from the actual problem.
...
The two italic steps call no model. The LLM is woken at four decision points only — which is how the token budget stays small, and token usage is scored.
